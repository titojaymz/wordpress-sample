...
class MiniMax_tabwidget extends WP_Widget
{
    ...
    {
        parent::__construct('minimax_tabwidget', 'Tabs', array('description' => 'Tab Panel'));
        if (is_admin()) {
    ...
    function widget($args, $instance)
    {
        ...
        <style> .nav-tabs li { list-style: none !important; } </style>
        <?php
    ...
    function form($instance)
    {
        ...
        <div style="padding-top: 0;" id="poststuff" class="left_box postbox ">
            ...
            <ul class="tab_ul" id="inactive_tabs">
                ...
                foreach ($tab_posts as $key => $tab_post) {
                    ...
                    if ($flag == 0)
                        echo "<li class='ui-state-default' rel='" . $tab_post->ID . "' id='p_" . $tab_post->ID . "'><span style='padding:0px;margin:0px;'>" . $tab_post->post_title . "<small class='tab-small'></small></span></li>";
                }
                ?>
            </ul>
        </div>
                ...
                if (!empty($pid)) {
                    for ($i = 0; $i < count($pid); $i++) {
                        ...
                        <li class='ui-state-default' rel='<?php echo $pid[$i]; ?>'>
                            <span style='padding:0px;margin:0px;'><?php echo $pimg->post_title; ?>
                                <small class='tab-small'></small></span>
        ...
        <p class="settings-info">Tabs are custom posts. Add new Tab from <b>Dashboard >> Tabs >> Add New</b> menu. Drag Tabs from left to right box to insert them into page.</p>
