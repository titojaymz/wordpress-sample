...
jQuery(function(){
    ...
    if(pageid != '') {
        var mxnb =  "<a class='ghbutton export ui-button' href='" + mx_admin_url + "?minimaxexport=" + pageid + "' ><span class='minimax-icons minimax-outbox'></span> Export Layout</a>" +
                    "<a class='ghbutton import-layout import ui-button' rel='" + pageid + "' href='#' ><span class='minimax-icons minimax-inbox'></span> Import Layout</a> " +
                    "<a class='ghbutton clone ui-button' href='" + mx_admin_url + "?minimaxclone=" + pageid + "' ><span class='minimax-icons minimax-clone'></span> Clone</a>";
    }
    else{
        var mxnb =  "<a class='ghbutton export ui-button' href='#' onclick='alert(\"Page/Post is not published or saved yet!\");return false;'><span class='minimax-icons minimax-outbox'></span> Export Layout</a>" +
                    "<a class='ghbutton import ui-button' href='#' onclick='alert(\"Page/Post is not published or saved yet!\");return false;'><span class='minimax-icons minimax-inbox'></span> Import Layout</a> " +
                    "<a class='ghbutton clone ui-button' href='#' onclick='alert(\"Page/Post is not published or saved yet!\");return false;'><span class='minimax-icons minimax-clone'></span> Clone</a>";
    }

    jQuery("#wp-content-wrap").append("<div id='minimax-builder' class='wp-editor-container' style='display:none'>" +
                                            "<div class='quicktags-toolbar minimax-toolbar'>" +
                                                "<div class='ghbutton-group'>" +
                                                    "<a class='insert-layout ghbutton ui-button ' holder='#layout_"+post_type+"' rel='col-1' href='#' >1 Col</a>" +
                                                    "<a class='insert-layout ghbutton ui-button ' holder='#layout_"+post_type+"' rel='col-2' href='#' >2 Cols</a>" +
                                                    "<a class='insert-layout ghbutton ui-button ' holder='#layout_"+post_type+"' rel='col-3' href='#' >3 Cols</a>" +
                                                    "<a class='insert-layout ghbutton ui-button ' holder='#layout_"+post_type+"' rel='col-4' href='#' >4 Cols</a>" +
                                                    "<a class='insert-layout ghbutton ui-button ' holder='#layout_"+post_type+"' rel='col-5' href='#' >5 Cols</a>" +
                                                    "<a class='insert-layout ghbutton ui-button ' holder='#layout_"+post_type+"' rel='col-6' href='#' >6 Cols</a>" +
                                                "</div>" +
                                                "<div class='ghbutton-group'>" +
                                                    mxnb +
                                                "</div>" +
    ...
    jQuery('#content-html,#content-tmce').click(function () {
        ...
        if (this.id == 'content-tmce') {
            jQuery('#ed_toolbar').hide();
            jQuery('#content-tmce').addClass('tactive');
        }
    ...
    //Import Layout
    var insertto = "", module_index = "", msf_mid = "", msf_title = "";
    jQuery('.import-layout').on('click', function () {
    ...
    jQuery('#module-settings-form').live('submit',function(){
        ...
        jQuery(this).ajaxSubmit({
            ...
            success:function(res){
                ...
                jQuery(insertto).append('<li id="module_'+module_index+'_'+z+'" rel="'+module_index+'">' +
                    '<input type="hidden" id="modid_module_'+module_index+'_'+z+'" name="modules['+module_index+'][]" value="'+msf_mid+'" />' +
                    '<input type="hidden" name="modules_settings['+module_index+'][]" id="modset_module_'+module_index+'_'+z+'" value="'+res+'" />' +
                    '<span class="module-title">'+msf_title+'</span>' +
                    '<div class="ctl">' +
                    '<div class="handle dtooltip" title="Drag this module"><span class="minimax-icons minimax-move"></span></div>' +
                    '<div class="delete_module dtooltip" title="Delete this module ?" rel="#module_'+module_index+'_'+z+'"><span class="minimax-icons minimax-cross"></span></div>' +
                    '<div class="insert dtooltip" title="Module Settings" rel="'+msf_mid+'" datafield="modset_module_'+module_index+'_'+z+'" data="'+module_index+'|0"><span class="minimax-icons minimax-cog"></span></div>' +
                    '<div class="module-clone dtooltip" title="Clone this module" col_id = '+module_index+' mod_name="'+msf_title+'" mod_id='+msf_mid+' mod_set= '+res+'><span class="minimax-icons minimax-clone"></span></div>' +
                    '</div>' +
                    '<div class="clear"></div>' +
                    '<div class="before-preview">' +
                    '</div><div class="module-preview w3eden"><img src="images/loading.gif" /> Loading Preview...</div><div class="clear"></div></li>');
    ...
    jQuery('.module-clone').live('click',function(){
        ...
        //Load Module Frame
        jQuery("#"+col_id + " .module").append('<li id="module_'+col_id+'_'+z+'" rel="'+col_id+'">' +
            '<input type="hidden" id="modid_module_'+col_id+'_'+z+'" name="modules['+col_id+'][]" value="'+msf_mid+'" />' +
            '<input type="hidden" name="modules_settings['+col_id+'][]" id="modset_module_'+col_id+'_'+z+'" value="'+res+'" />' +
            '<span class="module-title">'+msf_title+'</span>' +
            '<div class="ctl">' +
            '<div class="handle dtooltip" title="Drag this module"><span class="minimax-icons minimax-move"></span></div> ' +
            '<div class="delete_module dtooltip" title="Delete this module ?" rel="#module_'+col_id+'_'+z+'"><span class="minimax-icons minimax-cross"></span></div>' +
            '<div class="insert dtooltip" title="Module Settings" wname="'+msf_title+'" rel="'+msf_mid+'" datafield="modset_module_'+col_id+'_'+z+'" data="'+col_id+'|0"><span class="minimax-icons minimax-cog"></span></div>' +
            '<div class="module-clone dtooltip" title="Clone this module" col_id = '+col_id+' mod_name="'+msf_title+'" mod_id='+msf_mid+' mod_set= '+res+'><span class="minimax-icons minimax-clone"></span></div>' +
            '</div>' +
            '<div class="clear"></div><div class="before-preview"></div><div class="module-preview w3eden">' +
            '<img src="images/loading.gif" /> Loading Preview...</div>' +
            '<div class="clear"></div>' +
            '</li>');
        //Load Module Preview
    ...
    jQuery('#update-module-settings-form').live('submit', function () {
        ...
        jQuery(this).ajaxSubmit({
            ...
            success: function (res) {
                ...
                jQuery('#' + mod + ' .module-preview').load(ajaxurl + '?page=minimax&action=get_module_preview', {
                    mod: msf_mid,
                    modinfo: res
                });
            }
    ...
    // Drag using only drag icon ( .handle div )
    //jQuery('.module').sortable({handle: '.handle', connectWith: "ul.module"});
    // Drag clickng any where in module ( li.mod-single )
    jQuery('.module').sortable({connectWith: "ul.module"});
...
function load_layout(layout) {
    jQuery.get("admin-ajax.php?page=minimax&action=insert_layout&holder=" + holder_id + "&layout=" + layout, function (res) {
        ...
        /*Binding drag and drop feature in dynamically generated columns (v.3.1.1)*/
